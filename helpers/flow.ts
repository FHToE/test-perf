...
export async function step6_ensureTargetRoom(
  ...
  ctx.log(`step6: current URL ${ctx.page.url()} ≠ target '${targetRoomName}' — opening minimap`);

  // Open the minimap (toggle MapExpand button)
  try {
    await ctx.page.locator(".map-expand-control").click({ timeout: 5_000 });
    await ctx.page.waitForSelector(".minimap-container.expanded", { timeout: 5_000 });
...
export async function step7_findAndClickExhibit(ctx: FlowContext, holdMs: number): Promise<boolean> {
  const room = ctx.roomData.getLatest();
  ...
  // Verify the click actually opened the DetailedPoi dialog (Radix sets
  // [role="dialog"][data-state="open"]). If it didn't, the scan-coords were
  // a false positive — don't waste holdMs polluting target_room_poi_open
  // metrics with "empty canvas with no dialog" frames.
  const dialogOpened = await ctx.page
    .waitForSelector('[role="dialog"][data-state="open"]', { timeout: 3_000 })
    .then(() => true)
    .catch(() => false);

  if (!dialogOpened) {
    ctx.log("step7: clicked POI but dialog never opened — false-positive cursor-scan");
    ctx.notes.push("step7: false-positive POI scan (dialog never opened)");
  ...
  // Close the DetailedPoi dialog so it doesn't block subsequent step8 portal search.
  // Core's Dialog is Radix-based — Escape dismisses it.
...
/**
 * Close the DetailedPoi (Radix Dialog). Tries Escape first; falls back to clicking
 * outside the dialog. Safe to call when no dialog is open — no-op.
 */
async function closeOpenPoiDialog(ctx: FlowContext): Promise<void> {
  // Press Escape — Radix Dialog closes on Esc by default
  await ctx.page.keyboard.press("Escape").catch(() => undefined);
  await ctx.page.waitForTimeout(400);
  // Verify nothing is still blocking the canvas
  const stillOpen = await ctx.page
    .evaluate(() => !!document.querySelector('[role="dialog"][data-state="open"]'))
    .catch(() => false);
  if (stillOpen) {
    ctx.log("step7: POI dialog still open after Escape — clicking corner of viewport");
    // Click top-right corner where the close X usually sits
    await ctx.page.mouse.click(1240, 200).catch(() => undefined);
    await ctx.page.waitForTimeout(400);
  }
